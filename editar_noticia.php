...
if (isset($_REQUEST['editGame'])) {
    $consultaDatosN = "SELECT * FROM noticia WHERE idN = {$_REQUEST['idN']}";
    $runDatosN = mysqli_query($conexion, $consultaDatosN);
    $rowDatosN = mysqli_fetch_assoc($runDatosN);
}

if (isset($_REQUEST['subirCambiosN'])) {
    if (($_REQUEST['tituloN'] == "") || ($_REQUEST['descripN'] == "") || ($_REQUEST['autorN'] == "") || ($_REQUEST['fechaN'] == "")) {
        ...
        $actualizar = "UPDATE noticia SET tituloN= '$tituloN', descripN= '$descripN',  autorN= '$autorN',  fechaN= '$fechaN' WHERE idN = {$_REQUEST['idN']}";

        if (isset($_FILES['fotoN'])) {
            $fotoN = $_FILES['fotoN'];
            $estadoFoto = 1; // estado de subida del formulario | Estados posibles ==> 0 = Error / 1 = Subir formulario

            $name_file = "img_noticia_" . $_REQUEST['idN'];
...
<body class="hold-transition sidebar-mini layout-fixed dark-mode">
                    ...
                    <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                        ...
                        <li class="nav-item menu-open">
                            ...
                            <ul class="nav nav-treeview">
                                ...
                                <li class="nav-item">
                                    <a href="editar_noticia.php" class="nav-link active">
                                        <i class="nav-icon fas fa-edit"></i>
                                        <p>Editar noticia</p>
                                    </a>
                                </li>
                        ...
                        <div class="col-4 mx-auto">
                            <div class="card card-primary">
                                <div class="card-header">
                                    <h3 class="card-title">Ingrese la Información de la Noticia</h3>
                                </div>
                                <!-- /.card-header -->
                                <div class="card-body">
                                    <?php
                                    ...
                                    <form class="mx-5 mt-4 needs-validation" novalidate action="<?php htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST" enctype="multipart/form-data">
                                        <div class="card-body">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <textarea class="form-control" id="descripN" name="descripN" rows="5" placeholder="" required><?php if (isset($rowDatosN['descripN'])) {
                                                                                                                                                    echo $rowDatosN['descripN'];
                                                                                                                                                } ?></textarea>
                                            ...
                                            <div class="form-group" class="form-label">
                                                <label for="fotoN">Foto de la Noticia</label>
                                                <input type="file" class="form-control-file" id="fotoN" name="fotoN" placeholder="" disabled>
                                                <div class="invalid-feedback">Por favor ingrese una imagen para la noticia</div>
                                            </div>
                                        </div>
                                        <!-- /.card-body -->
                                        <div class="card-footer">
                                            <input type='hidden' name='idN' value=" <?php echo $rowDatosN['idN'] ?>">
                                            <button type="submit" name="subirCambiosN" class="btn btn-primary">Editar Noticia</button>
                                        </div>
                                    </form>
                                    </div">
                                </div>
    ...
    <script>
        document.getElementById('cambiarImagen').onchange = function() {
            document.getElementById('fotoN').disabled = !this.checked;
            document.getElementById('fotoN').required = this.checked;
        };
    </script>
    <script>
